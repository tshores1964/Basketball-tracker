...
let screen      = "home";
let aboutTab    = "coaches";
let curPlayer   = null;
let coachOpen   = false;
let coachTab    = "dashboard";
let coachViewPlayer = null;
let coachCommentDay = 0;
let pinEntry    = "";
let pinErr      = "";
let pinMode     = "coach";
let pinSetupFirst = "";
...
async function saveShot(player,week,cat,spot,day,made,att) {
  ...
  if(existing){const{error}=await db.from("shots").update({made,attempts:att}).eq("id",existing.id);if(!error){existing.made=made;existing.attempts=att;}return!error;}
  else{const{data,error}=await db.from("shots").insert({player,week,category:cat,spot,day,made,attempts:att,team_code:teamCode}).select().single();if(!error&&data)allShots.push(data);return!error;}
}
...
function buildRoster() {
  var items="";
  if(roster.length===0){items='<p style="color:#888;font-size:13px;padding:6px 0">No players yet.</p>';}
  else{roster.forEach(function(n){const hasPin=getPlayerPin(n)!==null,playerComment=getCoachComment(n,"player",null,null);items+='<div class="roster-item"><div style="display:flex;align-items:center;gap:9px"><div class="avatar">'+initials(n)+'</div><div><div>'+h(n)+'</div><div style="font-size:10px;color:'+(hasPin?'#888':'#2E75B6')+'">'+(hasPin?'🔒 PIN set':'🔑 No PIN yet')+'</div>'+(playerComment?'<div style="font-size:10px;color:#856404;margin-top:1px">Has coach note</div>':"")+'</div></div><div style="display:flex;flex-direction:column;gap:4px;align-items:flex-end"><button class="btn-sm btn-primary" data-action="coach-view-player" data-name="'+h(n)+'" style="font-size:11px;width:100%">💬 Comments</button><div style="display:flex;gap:4px">'+(hasPin?'<button class="btn-sm" data-action="reset-player-pin" data-name="'+h(n)+'" style="font-size:10px">Reset PIN</button>':"")+'<button class="btn-sm btn-danger" data-action="rm-player" data-name="'+h(n)+'">🗑</button></div></div></div>';});}
  return '<div class="card"><h3>Roster ('+roster.length+')</h3>'+items+'</div>'
    +'<div class="card"><h3>Add player</h3><div class="row-flex"><input type="text" id="np" placeholder="Player name" style="flex:1" /><button data-action="add-player" class="btn-primary">Add</button></div><div id="rmsg"></div></div>';
}
function buildCoachPlayerView() {
  ...
  return '<div style="display:flex;align-items:center;gap:10px;margin-bottom:14px"><button data-action="coach-back-roster" style="font-size:12px">Back to Roster</button><div class="avatar avatar-lg">'+initials(name)+'</div><div style="flex:1"><div style="font-weight:500;font-size:15px">'+h(name)+'</div><div style="font-size:11px;color:#888">This week: '+(tot.pct===null?"no data":tot.pct+"% ("+tot.m+"/"+tot.a+")")+'</div></div></div>'
    +'<div class="card" style="padding:16px;margin-bottom:10px"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:4px">Player Note</div><div style="font-size:11px;color:#888;margin-bottom:8px">Stays on their profile until you update it.</div><textarea id="coach-player-note" placeholder="e.g. Great attitude this week. Keep working on left-hand finishes." style="width:100%;min-height:70px;padding:8px 10px;border:1px solid #ccc;border-radius:8px;font-size:13px;font-family:inherit;resize:vertical;background:#fafafa">'+h(playerCmnt?playerCmnt.text:"")+'</textarea><button data-action="save-coach-comment" data-type="player" data-week="" data-day="" class="btn-primary" style="width:100%;padding:10px;margin-top:8px;font-size:13px">Save Player Note</button></div>'
    +'<div class="card" style="padding:16px;margin-bottom:10px"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:4px">Week Note</div><div style="font-size:11px;color:#888;margin-bottom:8px">For week of '+fmtWeek(wk)+'.</div><textarea id="coach-week-note" placeholder="e.g. Good effort this week. Focus on form next week." style="width:100%;min-height:70px;padding:8px 10px;border:1px solid #ccc;border-radius:8px;font-size:13px;font-family:inherit;resize:vertical;background:#fafafa">'+h(weekCmnt?weekCmnt.text:"")+'</textarea><button data-action="save-coach-comment" data-type="week" data-week="'+wk+'" data-day="" class="btn-primary" style="width:100%;padding:10px;margin-top:8px;font-size:13px">Save Week Note</button></div>'
    +'<div class="card" style="padding:16px;margin-bottom:10px"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:4px">Day Note</div><div style="font-size:11px;color:#888;margin-bottom:8px">Pick a day then leave your note.</div><div style="display:flex;gap:4px;margin-bottom:10px;overflow-x:auto">'+dayBtns+'</div><div style="font-size:12px;color:#888;margin-bottom:6px">'+dayLabels[coachCommentDay]+'</div><textarea id="coach-day-note" placeholder="e.g. Looked tired today. Good pull-up game from the right side." style="width:100%;min-height:70px;padding:8px 10px;border:1px solid #ccc;border-radius:8px;font-size:13px;font-family:inherit;resize:vertical;background:#fafafa">'+h(dayCmnt?dayCmnt.text:"")+'</textarea><button data-action="save-coach-comment" data-type="day" data-week="'+wk+'" data-day="'+coachCommentDay+'" class="btn-primary" style="width:100%;padding:10px;margin-top:8px;font-size:13px">Save Day Note</button></div>'
...
function buildPlayerCategoryEditor(player) {
  const cats=getPlayerCats(player),hasOverride=allPlayerCategories.filter(function(c){return c.player===player&&c.team_code===teamCode;}).length>0,playerCatRows=allPlayerCategories.filter(function(c){return c.player===player&&c.team_code===teamCode;});
  var rows="";
  for(var i=0;i<cats.length;i++){const dbRow=playerCatRows[i],id=dbRow?dbRow.id:"";rows+='<div style="display:grid;grid-template-columns:1fr 70px 36px;gap:6px;align-items:center;margin-bottom:6px"><input type="text" id="pcat-name-'+i+'" value="'+h(cats[i].name)+'" style="padding:7px 9px;border:1px solid #ccc;border-radius:7px;font-size:12px;font-family:inherit;background:#fafafa" /><input type="number" id="pcat-weight-'+i+'" value="'+cats[i].weight+'" min="0.1" max="10" step="0.5" style="padding:7px 4px;border:1px solid #ccc;border-radius:7px;font-size:12px;text-align:center;background:#fafafa" /><button data-action="delete-player-cat" data-player="'+h(player)+'" data-idx="'+i+'" data-id="'+id+'" class="btn-sm btn-danger" style="padding:6px">X</button></div>';}
  return '<div class="card" style="padding:16px;margin-bottom:10px"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:4px">'+h(player)+"'s Categories</div>"
    ...
    +'<div style="display:grid;grid-template-columns:1fr 70px 36px;gap:6px;margin-bottom:6px"><div style="font-size:10px;color:#aaa;font-weight:500">Category</div><div style="font-size:10px;color:#aaa;font-weight:500;text-align:center">Wt</div><div></div></div>'
    +rows
    ...
    +'<div style="display:grid;grid-template-columns:1fr 70px;gap:6px;margin-top:10px;padding-top:10px;border-top:0.5px solid #eee"><input type="text" id="new-pcat-name" placeholder="Add category..." style="padding:7px 9px;border:1px solid #ccc;border-radius:7px;font-size:12px;font-family:inherit" /><input type="number" id="new-pcat-weight" value="1.0" min="0.1" max="10" step="0.5" style="padding:7px 4px;border:1px solid #ccc;border-radius:7px;font-size:12px;text-align:center" /></div>'
    ...
    +'<button data-action="reset-player-cats" data-player="'+h(player)+'" style="width:100%;padding:7px;margin-top:4px;font-size:11px;color:#888">Reset to team defaults</button></div>';
}
function buildCoachCategories() {
  ...
  var rows="";
  for(var i=0;i<cats.length;i++){rows+='<div style="display:grid;grid-template-columns:1fr 80px 40px;gap:8px;align-items:center;margin-bottom:8px"><input type="text" id="cat-name-'+i+'" value="'+h(cats[i].name)+'" style="padding:8px 10px;border:1px solid #ccc;border-radius:8px;font-size:13px;font-family:inherit;background:#fafafa" /><input type="number" id="cat-weight-'+i+'" value="'+cats[i].weight+'" min="0.1" max="10" step="0.5" style="padding:8px 6px;border:1px solid #ccc;border-radius:8px;font-size:13px;text-align:center;background:#fafafa" /><button data-action="delete-category" data-idx="'+i+'" data-id="'+(cats[i].id||"")+'" class="btn-sm btn-danger" style="padding:8px">X</button></div>';}
  return '<div class="card"><h3>Shooting Categories</h3><p style="font-size:12px;color:#888;margin-bottom:12px">Rename categories, adjust King Point weights, or add your own.</p><div style="display:grid;grid-template-columns:1fr 80px 40px;gap:8px;margin-bottom:6px"><div style="font-size:10px;color:#aaa;font-weight:500">Category Name</div><div style="font-size:10px;color:#aaa;font-weight:500;text-align:center">King Pts</div><div></div></div>'+rows+'<button data-action="save-categories" class="btn-primary" style="width:100%;padding:11px;margin-top:4px;font-size:13px;font-weight:500">Save Categories</button></div>'
    +'<div class="card"><h3>Add Category</h3><div style="display:grid;grid-template-columns:1fr 80px;gap:8px;margin-bottom:8px"><input type="text" id="new-cat-name" placeholder="e.g. Mid-Range Pull-Up" style="padding:8px 10px;border:1px solid #ccc;border-radius:8px;font-size:13px;font-family:inherit" /><input type="number" id="new-cat-weight" value="1.0" min="0.1" max="10" step="0.5" style="padding:8px 6px;border:1px solid #ccc;border-radius:8px;font-size:13px;text-align:center" /></div><div style="font-size:10px;color:#888;margin-bottom:8px">King Points weight - higher = worth more on the leaderboard</div><button data-action="add-category" class="btn-primary" style="width:100%;padding:11px;font-size:13px">+ Add Category</button><div id="cat-msg" style="margin-top:6px"></div></div>'
    +'<div class="card" style="background:#FFF9E6;border:1px solid #FFD700"><div style="font-size:12px;font-weight:500;color:#856404;margin-bottom:6px">King Points Reference</div><div style="font-size:11px;color:#555;line-height:1.7">Higher weight = harder shot = more King Points. Suggested: Easy = 0.5, Standard = 1.0, Pull-up = 2.0, 3pt = 3.0, Pull-up 3 = 4.0</div></div>';
}
function buildCoachMessages() {
  var msgList="";
  if(allMessages.length===0){msgList='<div style="text-align:center;padding:20px;color:#888;font-size:13px">No messages sent yet.</div>';}
  else{allMessages.forEach(function(m){const reads=allMessageReads.filter(function(r){return r.message_id===m.id;}).length,target=m.target_player||"Everyone",total=m.target_player?1:roster.length,diff=Date.now()-new Date(m.created_at).getTime(),mins=Math.floor(diff/60000),hrs=Math.floor(diff/3600000),days=Math.floor(diff/86400000),timeAgo=days>0?days+"d ago":hrs>0?hrs+"h ago":mins>0?mins+"m ago":"Just now",readColor=reads===total?"#27500A":"#856404",readBg=reads===total?"#E6F7EC":"#FFF9E6",unread=m.target_player?[]:roster.filter(function(p){return !allMessageReads.find(function(r){return r.message_id===m.id&&r.player===p;});}),unreadLine=unread.length>0?'<div style="font-size:10px;color:#888;margin-top:4px">Not yet read: '+unread.map(h).join(", ")+'</div>':"";msgList+='<div style="padding:12px 14px;border:0.5px solid #e0e0e0;border-radius:10px;margin-bottom:8px;background:#fafafa"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px"><div><div style="font-size:12px;color:#888">'+timeAgo+'</div><div style="font-size:10px;color:#1A3A5C;font-weight:500;margin-top:2px">To: '+h(target)+'</div></div><div style="display:flex;align-items:center;gap:6px"><div style="font-size:11px;color:'+readColor+';background:'+readBg+';padding:2px 8px;border-radius:10px">'+reads+"/"+total+' read</div><button data-action="delete-message" data-id="'+m.id+'" style="font-size:11px;color:#A32D2D;background:none;border:none;padding:2px 4px;cursor:pointer">X</button></div></div><div style="margin-bottom:4px">'+renderMessageText(m.text)+'</div>'+unreadLine+'</div>';});}
  const playerOpts='<option value="">Everyone</option>'+roster.map(function(n){return'<option value="'+h(n)+'">'+h(n)+'</option>';}).join("");
  return '<div class="card"><h3>Send Message</h3><p style="font-size:12px;color:#888;margin-bottom:10px">Paste a YouTube link to send a video. Player sees it as a banner when they open the app.</p><div style="margin-bottom:10px"><label style="font-size:12px;font-weight:500;color:#1A3A5C;display:block;margin-bottom:4px">Send to:</label><select id="msg-target" style="width:100%;padding:9px 10px;border:1px solid #ccc;border-radius:8px;font-size:13px;font-family:inherit;background:#fafafa">'+playerOpts+'</select></div><textarea id="msg-text" placeholder="Type a message or paste a YouTube link..." style="width:100%;min-height:80px;padding:10px;border:1px solid #ccc;border-radius:8px;font-size:13px;font-family:inherit;resize:vertical;background:#fafafa;margin-bottom:10px"></textarea><button data-action="send-message" class="btn-primary" style="width:100%;padding:12px;font-size:14px;font-weight:500">Send</button></div>'
...
function buildPlayer() {
  ...
  if(isMobile){
    ...
    for(var di2=0;di2<7;di2++){
      var dm=0,da=0;
      playerCats.forEach(function(pc){const nS=getSpotCount(name,pc.name);for(var si2=0;si2<nS;si2++){const v=getShot(name,wk,pc.name,si2,di2);dm+=parseInt(v.m)||0;da+=parseInt(v.a)||0;}});
    ...
    playerCats.forEach(function(pc,catIdxM){
      ...
      html+='<div class="cat-hdr" style="display:flex;align-items:center;justify-content:space-between"><span>'+h(cat)+'</span><span style="display:flex;gap:4px;align-items:center"><button data-action="spot-minus" data-catidx="'+catIdxM+'" style="background:rgba(255,255,255,.2);color:#fff;border:none;width:26px;height:26px;border-radius:4px;font-size:15px;font-weight:500;cursor:pointer">-</button><span style="font-size:10px;color:#fff;min-width:38px;text-align:center">'+nSpots+' spots</span><button data-action="spot-plus" data-catidx="'+catIdxM+'" style="background:rgba(255,255,255,.2);color:#fff;border:none;width:26px;height:26px;border-radius:4px;font-size:15px;font-weight:500;cursor:pointer">+</button></span></div>'
        +'<div class="card" style="padding:.75rem 1rem"><div style="display:grid;grid-template-columns:78px 1fr 1fr 50px;gap:6px;align-items:center;margin-bottom:6px"><div style="font-size:10px;color:#aaa;font-weight:500">Spot</div><div style="font-size:10px;color:#aaa;font-weight:500;text-align:center">Made</div><div style="font-size:10px;color:#aaa;font-weight:500;text-align:center">Attempts</div><div style="font-size:10px;color:#aaa;font-weight:500;text-align:center">%</div></div>';
      for(var si=0;si<nSpots;si++){
        ...
        html+='<div style="display:grid;grid-template-columns:78px 1fr 1fr 50px;gap:6px;align-items:center;margin-bottom:8px"><input type="text" data-spot-label-cat="'+h(cat)+'" data-spot-label-si="'+si+'" placeholder="Spot '+(si+1)+'" value="'+h(customLabel)+'" style="font-size:11px;color:#1A3A5C;font-weight:500;padding:6px 4px;border:0.5px solid #e0e0e0;border-radius:6px;background:#f5f7fa;width:100%" /><input type="number" min="0" max="99" inputmode="numeric" placeholder="0" value="'+h(val.m)+'" data-cat="'+h(cat)+'" data-si="'+si+'" data-di="'+selectedDay+'" data-f="m" style="padding:10px;text-align:center;font-size:16px;font-weight:500;border:1px solid #ccc;border-radius:8px;width:100%" /><input type="number" min="0" max="99" inputmode="numeric" placeholder="0" value="'+h(val.a)+'" data-cat="'+h(cat)+'" data-si="'+si+'" data-di="'+selectedDay+'" data-f="a" style="padding:10px;text-align:center;font-size:16px;font-weight:500;border:1px solid #ccc;border-radius:8px;background:#fafafa;width:100%" /><div class="pct-pill '+pctClass(p)+'" id="pp-'+cat.replace(/\W/g,"_")+'-'+si+'" style="font-size:12px;padding:6px">'+(p===null?"--":p+"%")+'</div></div>';
      }
      html+='</div>';
    });
  } else {
    ...
    for(var dni=0;dni<7;dni++){
      ...
      html+='<div><div style="font-size:10px;color:#888;font-weight:500;margin-bottom:3px">'+DAYS[dni]+'</div><textarea data-note-day="'+dni+'" placeholder="Notes..." style="width:100%;min-height:50px;padding:5px 6px;border:0.5px solid #ddd;border-radius:5px;font-size:11px;font-family:inherit;resize:none;background:#fafafa">'+h(nt)+'</textarea></div>';
    }
    html+='</div></div>';
    playerCats.forEach(function(pc,catIdxD){
      const cat=pc.name,nSpots=getSpotCount(name,cat);
      html+='<div class="cat-hdr" style="display:flex;align-items:center;justify-content:space-between"><span>'+h(cat)+'</span><span style="display:flex;gap:4px;align-items:center"><button data-action="spot-minus" data-catidx="'+catIdxD+'" style="background:rgba(255,255,255,.2);color:#fff;border:none;width:22px;height:22px;border-radius:4px;font-size:13px;cursor:pointer">-</button><span style="font-size:10px;color:#fff;min-width:40px;text-align:center">'+nSpots+' spots</span><button data-action="spot-plus" data-catidx="'+catIdxD+'" style="background:rgba(255,255,255,.2);color:#fff;border:none;width:22px;height:22px;border-radius:4px;font-size:13px;cursor:pointer">+</button></span></div>'
      ...
      for(var si=0;si<nSpots;si++){
        ...
        for(var di=0;di<7;di++){
          ...
          html+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:2px"><input type="number" min="0" max="99" placeholder="M" value="'+h(val.m)+'" data-cat="'+h(cat)+'" data-si="'+si+'" data-di="'+di+'" data-f="m" style="padding:4px 2px;text-align:center;font-size:11px;border:0.5px solid #ddd;border-radius:3px;width:100%" /><input type="number" min="0" max="99" placeholder="A" value="'+h(val.a)+'" data-cat="'+h(cat)+'" data-si="'+si+'" data-di="'+di+'" data-f="a" style="padding:4px 2px;text-align:center;font-size:11px;border:0.5px solid #ddd;border-radius:3px;background:#f9f9f9;width:100%" /></div>';
        }
...
function buildProgressChart(playerName){
  ...
  const catData={},pCats=getPlayerCats(playerName);
  ...
  const W=340,H=200,padL=40,padR=14,padT=14,padB=30,chartW=W-padL-padR,chartH=H-padT-padB,xStep=weeks.length>1?chartW/(weeks.length-1):0;
  ...
  var allLines="";
  pCats.forEach(function(pc,ci){const cat=pc.name,color=CAT_COLORS[cat]||["#888","#2E75B6","#1A3A5C","#27500A","#1E8449","#B8860B"][ci]||"#888",pts=catData[cat].map(function(d,i){return d.pct!==null?{x:xPos(i),y:yPct(d.pct)}:null;}).filter(function(p){return p;});if(!pts.length)return;allLines+='<polyline points="'+pts.map(function(p){return p.x+","+p.y;}).join(" ")+'" fill="none" stroke="'+color+'" stroke-width="1.8" />';allLines+=pts.map(function(p){return '<circle cx="'+p.x+'" cy="'+p.y+'" r="2.5" fill="'+color+'" />';}).join("");});
  ...
  return '<div class="card" style="padding:14px 16px"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:10px">Weekly % by Category</div><svg viewBox="0 0 '+W+' '+H+'" style="width:100%;height:auto;display:block">'+yGridLines+allLines+xLabels+'</svg><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:10px;padding-top:10px;border-top:0.5px solid #eee">'+legend+'</div><div style="font-size:10px;color:#888;margin-top:8px;text-align:center;font-style:italic">In-gym shooting % is the ceiling - what you can do here predicts what you will do in a game</div></div>';
}
function buildCoachFeedback(playerName) {
  const wk=weekKey(),playerCmnt=getCoachComment(playerName,"player",null,null),weekCmnt=getCoachComment(playerName,"week",wk,null);
...
function buildDailyCheckin() {
  ...
  return '<div style="max-width:520px;margin:0 auto"><div style="text-align:center;margin-bottom:18px"><div style="font-size:11px;color:#888;letter-spacing:1px;text-transform:uppercase;font-weight:500">Daily Check-In</div><div style="font-size:13px;color:#1A3A5C;margin-top:4px">'+dayLabel+' - '+h(name)+'</div></div>'
    ...
    +'<div class="card" style="padding:18px 16px;margin-bottom:12px"><div style="font-size:14px;font-weight:500;color:#1A3A5C;margin-bottom:12px">When something went wrong, what did you do?</div><div style="display:grid;gap:6px">'+btn("recovery","Flushed it","Flushed it and moved on")+btn("recovery","Got down a little","Got down a little but recovered")+btn("recovery","Stayed down","Got down and stayed down")+'</div></div>'
    +'<div class="card" style="padding:18px 16px;margin-bottom:14px"><div style="font-size:14px;font-weight:500;color:#1A3A5C;margin-bottom:12px">One word for how you felt today?</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">'+["Locked In","Solid","Off","Frustrated","Tired","Confident"].map(function(f){return btn("feeling",f,f);}).join("")+'</div></div>'
    +'<div style="display:flex;gap:8px"><button data-action="skip-checkin" style="flex:1;padding:12px;font-size:13px;color:#888">Skip for now</button><button data-action="save-checkin" class="btn-primary" style="flex:2;padding:12px;font-size:14px;font-weight:500">Save Check-In</button></div></div>';
}
function buildWeeklyCheckin() {
  ...
  return '<div style="max-width:520px;margin:0 auto"><div style="text-align:center;margin-bottom:18px"><div style="font-size:11px;color:#888;letter-spacing:1px;text-transform:uppercase;font-weight:500">Weekly Check-In</div><div style="font-size:13px;color:#1A3A5C;margin-top:4px">Week of '+fmtWeek(wk)+' - '+h(name)+'</div></div>'
    +'<div class="card" style="padding:18px 16px;margin-bottom:12px"><div style="font-size:14px;font-weight:500;color:#1A3A5C;margin-bottom:8px">Did your actions this week match your goals?</div><div style="font-size:11px;color:#888;margin-bottom:14px">Slide honestly. 1 = not at all. 10 = locked in all week.</div><div style="display:flex;align-items:center;gap:12px"><input type="range" id="alignment-slider" min="1" max="10" value="'+(ex.alignment||5)+'" style="flex:1" oninput="document.getElementById(\'alignment-val\').textContent=this.value" /><div id="alignment-val" style="font-size:22px;font-weight:500;color:#1A3A5C;min-width:30px;text-align:center">'+(ex.alignment||5)+'</div></div></div>'
    +'<div class="card" style="padding:18px 16px;margin-bottom:12px"><div style="font-size:14px;font-weight:500;color:#1A3A5C;margin-bottom:12px">Did you compete with confidence this week?</div><div style="display:grid;gap:6px">'+wbtn("confidence","Full confidence","Full confidence - I trusted my work")+wbtn("confidence","Some confidence","Some confidence in spots")+wbtn("confidence","In my head","In my head more than I should have been")+'</div></div>'
    +'<div class="card" style="padding:18px 16px;margin-bottom:14px"><div style="font-size:14px;font-weight:500;color:#1A3A5C;margin-bottom:12px">Rate your self-talk this week.</div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px">'+stbtn("Positive")+stbtn("Mixed")+stbtn("Negative")+'</div></div>'
    +'<div style="display:flex;gap:8px"><button data-action="go-home" style="flex:1;padding:12px;font-size:13px;color:#888">Cancel</button><button data-action="save-wcheckin" class="btn-primary" style="flex:2;padding:12px;font-size:14px;font-weight:500">Save Weekly Check-In</button></div></div>';
}
function buildSummary() {
  ...
  pCats.forEach(function(pc){const cat=pc.name,nSpots=getSpotCount(name,cat);for(var si=0;si<nSpots;si++){var sm=0,sa=0;for(var di=0;di<7;di++){const v=getShot(name,wk,cat,si,di);sm+=parseInt(v.m)||0;sa+=parseInt(v.a)||0;}if(sa>=10){const p=Math.round(sm/sa*100);if(!bestSpot||p>bestSpot.pct){bestSpot={cat:cat,label:getSpotLabel(name,cat,si)||"Spot "+(si+1),pct:p,m:sm,a:sa};}}}});
  function periodCard(label,stats,color){return '<div class="card" style="padding:14px 16px;background:linear-gradient(135deg,'+color+'10,transparent)"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><div style="font-size:11px;color:#888;letter-spacing:.5px;text-transform:uppercase;font-weight:500">'+label+'</div><div style="font-size:10px;color:#888">'+stats.wkCount+' week'+(stats.wkCount===1?'':'s')+'</div></div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;text-align:center"><div><div style="font-size:22px;font-weight:500;color:'+color+'">'+(stats.tot.pct===null?"--":stats.tot.pct+"%")+'</div><div style="font-size:10px;color:#888">Shooting %</div></div><div><div style="font-size:22px;font-weight:500;color:#1A3A5C">'+stats.tot.m+'</div><div style="font-size:10px;color:#888">Makes</div></div><div><div style="font-size:22px;font-weight:500;color:#1A3A5C">'+stats.tot.a+'</div><div style="font-size:10px;color:#888">Attempts</div></div></div></div>';}
  var catRows="";
  ...
  return '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;flex-wrap:wrap"><button data-action="go-home">\u2190 Back</button><div class="avatar avatar-lg">'+initials(name)+'</div><div style="flex:1"><div style="font-weight:500;font-size:15px">'+h(name)+'</div><div style="font-size:11px;color:#888">Summary</div></div>'+trendHtml+streakHtml+'</div>'
    ...
    +'<div class="card" style="padding:14px 16px"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:10px">By Category</div><table class="dash"><thead><tr><th style="text-align:left">Category</th><th>Week</th><th>Month</th><th>Year</th></tr></thead><tbody>'+catRows+'</tbody></table></div>';
}
async function buildLeague() {
  ...
  if(leagueTeams.length === 0) {
    render(`<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"><button data-action="go-home">← Back</button><span style="font-weight:500;font-size:15px">🌐 League</span></div><div class="card" style="text-align:center;padding:30px 20px"><div style="font-size:32px;margin-bottom:10px">🏀</div><div style="font-size:15px;font-weight:500;color:#1A3A5C;margin-bottom:8px">No teams in the league yet</div><div style="font-size:12px;color:#888">Coaches can join the league from Coach Panel → Settings.</div></div>`);
    return;
  }
  const teamStats = leagueTeams.map(team => {
    ...
    let kingPts = 0;
    CATS.forEach(cat => {const w = weightMap[cat] || 1.0;const catMade = shots.filter(s=>s.category===cat).reduce((a,s)=>a+(s.made||0),0);kingPts += catMade * w;});
  ...
  if(teamStats.length === 0) {
    render(`<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"><button data-action="go-home">← Back</button><span style="font-weight:500;font-size:15px">🌐 League</span></div><div class="card" style="text-align:center;padding:30px 20px"><div style="font-size:32px;margin-bottom:10px">🏀</div><div style="font-size:15px;font-weight:500;color:#1A3A5C;margin-bottom:8px">${leagueTeams.length} team${leagueTeams.length===1?'':'s'} in the league</div><div style="font-size:12px;color:#888">No shots logged this week yet. Check back after practice!</div></div>`);
    return;
  }
  const champion = teamStats[0];
  const medals = ["🥇","🥈","🥉"],medalColors = ["#FFD700","#C0C0C0","#CD7F32"];
  const championBanner = `<div style="background:linear-gradient(135deg,#2a1a00,#1a0f00);border:1.5px solid #FFD700;border-radius:12px;padding:16px;margin-bottom:14px;text-align:center"><div style="font-size:10px;letter-spacing:1px;color:#FFD700;text-transform:uppercase;margin-bottom:6px">🏆 League Champion — Week of ${fmtWeek(wk)}</div><div style="font-size:24px;font-weight:500;color:#FFD700;margin-bottom:8px">${champion.name}</div><div style="display:flex;justify-content:center;gap:20px;flex-wrap:wrap"><div style="text-align:center"><div style="font-size:20px;font-weight:500;color:#FFD700">${champion.kingPts}</div><div style="font-size:10px;color:#888">King Points</div></div><div style="text-align:center"><div style="font-size:20px;font-weight:500;color:#fff">${champion.made}</div><div style="font-size:10px;color:#888">Total Makes</div></div><div style="text-align:center"><div style="font-size:20px;font-weight:500;color:#fff">${champion.pct===null?"—":champion.pct+"%"}</div><div style="font-size:10px;color:#888">Team Shooting %</div></div></div></div>`;
  const maxKing = teamStats[0].kingPts || 1;
  const rows = teamStats.map((t,i) => {const barW=Math.round((t.kingPts/maxKing)*100),medal=medals[i]||String(i+1),color=medalColors[i]||"#888";return `<div class="sb-row ${i<3?"medal-"+(i+1):""}"><div class="sb-rank" style="font-size:${i<3?'18':'13'}px">${medal}</div><div style="flex:1;min-width:0"><div style="font-weight:500;font-size:13px;color:#1A3A5C">${t.name}</div><div style="font-size:10px;color:#888">${t.made} makes · ${t.pct===null?"—":t.pct+"% shooting"}</div></div><div style="display:flex;align-items:center;gap:8px;min-width:120px"><div class="sb-bar-wrap" style="flex:1"><div class="sb-bar" style="width:${barW}%"></div></div><div style="text-align:right;min-width:40px"><div class="sb-stat" style="color:${color}">${t.kingPts}</div><div class="sb-sub">pts</div></div></div></div>`;}).join("");
  render(`<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"><button data-action="go-home">← Back</button><span style="font-weight:500;font-size:15px">🌐 League Standings</span>${teamCompete?'<span style="font-size:11px;color:#27500A;background:#E6F7EC;padding:3px 8px;border-radius:10px">✅ Your team is in</span>':''}</div>${championBanner}<div class="sb-wrap"><div class="sb-title">🏀 Team Rankings — Week of ${fmtWeek(wk)}</div><div style="font-size:11px;color:#888;margin-bottom:10px;padding:0 4px">Ranked by King Points (weighted makes)</div>${rows}</div><div style="text-align:center;margin-top:12px;font-size:11px;color:#888">${leagueTeams.length} team${leagueTeams.length===1?'':'s'} competing · Updates live</div>`);
}
function buildLeaderboard() {
  try {
    ...
    function ranked(arr){return arr.filter(function(x){return x.val!==null;}).sort(function(a,b){return(b.exact||b.val)-(a.exact||a.val);});}
    const kingPoints=roster.map(function(n){const w=playerWeightedMakes(n,weeks),pts=Math.round(w*10)/10,t=playerTotals(n,weeks),rk=getRank(pts);return{name:n,val:pts,sub:'<span style="color:'+rk.color+';font-weight:500">'+rk.icon+" "+rk.name+'</span> \u00b7 '+t.m+" made \u00b7 "+(t.pct===null?"--":t.pct+"%")};}).filter(function(p){return p.val>0;}).sort(function(a,b){return b.val-a.val;});
    ...
    const sectionIds=["king","streaks","overall","attempts","bestday","improved","cats"],sectionLabels=["\uD83D\uDC51 King Points","\uD83D\uDD25 Streaks","Overall %","Most Shots","Best Day","Improved","By Category"];
    ...
    let content="";
    if(sbSection==="king")     content='<div class="sb-section"><div class="sb-section-title">\uD83D\uDC51 King Points \u2014 weighted makes & rank</div>'+sbRows(kingPoints,true)+'<div style="font-size:10px;color:#888;margin-top:8px;text-align:center;font-style:italic">Harder shots are worth more. Rank resets every week. \uD83C\uDF31 Rookie \u2192 \uD83C\uDFAF Marksman (500) \u2192 \uD83C\uDFC0 Sharpshooter (1,000) \u2192 \uD83D\uDD2D Sniper (1,500) \u2192 \u26A1 Deadeye (2,000) \u2192 \uD83D\uDC51 Legend (2,500)</div></div>';
    ...
    const weekNav='<div style="display:flex;align-items:center;justify-content:center;gap:8px;margin:8px 0"><button data-action="sb-week-prev" '+(wIdx>=navWeeks.length-1?'disabled style="padding:6px 12px;font-size:13px;border-radius:7px;opacity:.35"':'style="padding:6px 12px;font-size:13px;border-radius:7px"')+'>\u25C0 Prev</button><div style="min-width:150px;text-align:center"><div style="font-size:12px;color:#1A3A5C;font-weight:500">'+fmtWeek(viewWeek)+'</div><div style="font-size:11px;color:#888">'+(isCurrentWeek?"This Week":"Previous Week")+'</div></div><button data-action="sb-week-next" '+(wIdx===0?'disabled style="padding:6px 12px;font-size:13px;border-radius:7px;opacity:.35"':'style="padding:6px 12px;font-size:13px;border-radius:7px"')+'>Next \u25B6</button></div>';
...
function buildAbout() {
  const coachFeatures=[["Shot Tracking","Players log makes and attempts by category, spot, and day — every workout, every week."],["Shooting King","A weighted leaderboard crowns each week's best shooter. Harder shots are worth more."],["Player Ranks","Weekly tiers from Rookie up to Legend, based on King Points. Resets every week so there's always a new climb."],["Streaks","Tracks each player's consecutive-day logging streak — rewards showing up, not just talent."],["Most Improved","Ranks players by growth from their first week to their latest, against their own baseline."],["Leaderboards","King Points, Ranks, Streaks, Overall %, Most Shots, Best Day, Most Improved, and by category."],["Messages & Video","Send a note or paste a YouTube drill to the whole team or one player. Videos play right in the app."],["Mental Performance","Daily and weekly check-ins built on CMPT principles — effort, recovery, confidence, self-talk."],["Coach Comments","Leave feedback per player, per week, or per day. Only that player sees it."],["Player Summaries","Week, month, and year stats. Best spot. Progress chart by category. Trend vs last week."],["Custom Categories","Rename drills, set King Point weights, and even customize categories per player."],["League Competition","Opt your team into cross-team competition against other Sharpshooter teams."],["Player PINs","Each player protects their own data with a personal PIN. You have a master override."]];
  const coachHowTo=[["Create your team","Tap Create New Team, name it, and set your coach PIN. You'll get a 6-character team code."],["Add your players","Coach panel → Roster tab → add each player by name."],["Set categories & weights","Categories tab — rename drills and set King Point weights. Harder shot, higher weight."],["Share your code","Give players the team code. They join, set a PIN, and start logging."],["Send messages & video","Messages tab — send a note or a YouTube drill to everyone or one player."],["Leave feedback","Roster → tap a player → Comments. Drop a note per player, week, or day."],["Recognize the work","Each week, open the Leaderboard and call out the King, top Ranks, longest Streaks, and Most Improved."]];
  const recognizeIdeas=["Crown the weekly Shooting King out loud at practice.","Shout out the longest current Streak — consistency wins.","Celebrate Most Improved, measured against their own start.","Call out anyone who reached a new Rank that week.","Rotate a Player of the Week using the Messages tab."];
  const kingScoring=[["Form Shooting","0.5x"],["Catch & Shoot","1.0x"],["Finishes","1.0x"],["1-Dribble Pull-Up","2.0x"],["Catch & Shoot 3s","3.0x"],["1-Dribble Pull-Up 3s","4.0x"]];
  const playerSteps=[["Join Your Team","Your coach gives you a team code. Type it in and you're on the roster."],["Set Your PIN","Pick a 4-digit PIN. Your data is yours — nobody else can change your numbers."],["Log Your Workout","Tap your name, pick the day, enter makes and attempts for each spot. Takes 2 minutes."],["Climb Your Rank","Every week starts at Rookie. Stack King Points to climb to Legend — then do it again next week."],["Keep Your Streak","Log every day to build your streak. The more days in a row, the higher you climb the Streaks board."],["Check the King Board","See who's leading in weighted shooting points. Can you take the crown?"],["Do Your Check-In","After saving, answer 3 quick questions about your mental game. 30 seconds."],["See Your Summary","Your rank, streak, best spot, progress chart, and your coach's feedback — all in one place."]];
  const rankLadder=RANK_LEVELS.map(function(r){return '<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 9px;background:#fff;border-radius:6px;border:0.5px solid #e0e0e0;margin-bottom:5px"><span style="font-size:12px;color:#444"><span style="font-size:14px">'+r.icon+'</span> '+r.name+'</span><span style="font-size:11px;font-weight:500;color:'+r.color+'">'+(r.min===0?"Start here":r.min+" King Pts")+'</span></div>';}).join("");
  const coachContent='<div style="padding:4px 0"><div style="background:linear-gradient(135deg,#1A3A5C,#0C2340);border-radius:12px;padding:20px 18px;margin-bottom:16px;text-align:center"><div style="font-size:32px;margin-bottom:8px">🎯</div><div style="font-size:20px;font-weight:500;color:#FFD700;margin-bottom:6px">Sharpshooter</div><div style="font-size:12px;color:#ccc;font-style:italic;margin-bottom:10px">"What gets measured, improves"</div><div style="display:inline-block;background:#FFD700;color:#1A3A5C;font-size:11px;font-weight:500;padding:4px 12px;border-radius:20px">MVP — Early Access</div></div><div class="card" style="border-left:3px solid #FFD700"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:8px">🏀 Built by a Coach, for Coaches</div><div style="font-size:13px;color:#444;line-height:1.7">Sharpshooter was built by Coach Todd Shores — 30+ years coaching youth basketball, CMPT, and a coach who got tired of players not tracking their work.<br><br>This is a <strong>culture tool</strong>. It rewards the work that wins games — the shooting workouts nobody sees.</div></div><div class="card"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:10px">⚙️ What It Does</div>'+coachFeatures.map(function(item){return'<div style="display:flex;gap:10px;margin-bottom:12px;align-items:flex-start"><div style="font-size:13px;font-weight:500;color:#1A3A5C;min-width:108px"><strong>'+item[0]+'</strong></div><div style="font-size:12px;color:#555;line-height:1.5">'+item[1]+'</div></div>';}).join("")+'</div><div class="card"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:10px">📋 How to Use It</div>'+coachHowTo.map(function(s,i){return'<div style="display:flex;gap:10px;margin-bottom:12px;align-items:flex-start"><div style="font-size:16px;min-width:24px;font-weight:500;color:#1A3A5C">'+(i+1)+'.</div><div><div style="font-size:13px;font-weight:500;color:#1A3A5C">'+s[0]+'</div><div style="font-size:12px;color:#555;line-height:1.5;margin-top:2px">'+s[1]+'</div></div></div>';}).join("")+'</div><div class="card" style="background:linear-gradient(135deg,#E6F1FB,#fff);border:1px solid #2E75B6"><div style="font-size:13px;font-weight:500;color:#0C447C;margin-bottom:10px">🏆 Ways to Recognize Players</div>'+recognizeIdeas.map(function(t){return'<div style="display:flex;gap:8px;margin-bottom:8px;align-items:flex-start"><div style="color:#2E75B6;font-size:13px;line-height:1.4">•</div><div style="font-size:12px;color:#444;line-height:1.5">'+t+'</div></div>';}).join("")+'</div><div class="card" style="background:linear-gradient(135deg,#FFF9E6,#fff);border:1px solid #FFD700"><div style="font-size:13px;font-weight:500;color:#856404;margin-bottom:10px">👑 The King Scoring System</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">'+kingScoring.map(function(r){return'<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 8px;background:#fff;border-radius:6px;border:0.5px solid #e8d77a"><span style="font-size:11px;color:#555">'+r[0]+'</span><span style="font-size:12px;font-weight:500;color:#856404">'+r[1]+'</span></div>';}).join("")+'</div><div style="font-size:10px;color:#888;margin-top:8px;font-style:italic">These are the defaults — change any weight in the Categories tab.</div></div><div class="card" style="background:#0a0f1a;border:1px solid #334"><div style="font-size:13px;font-weight:500;color:#FFD700;margin-bottom:8px">💰 Pricing — Early Access</div><div style="font-size:12px;color:#aaa;line-height:1.7">Free MVP version while we build and test with real teams. Full App Store launch coming — <strong style="color:#fff">$9.99/month per team</strong>.</div></div><div class="card" style="border:1px solid #1A3A5C"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:6px">📲 Get Started</div><ol style="font-size:12px;color:#444;line-height:2;padding-left:18px;margin:0"><li>Open <strong>basketball-tracker-nine.vercel.app</strong></li><li>Add to your home screen</li><li>Create your team and get a team code</li><li>Share the code with your players</li></ol></div><button data-action="share-coach" class="btn-primary" style="width:100%;padding:14px;font-size:14px;font-weight:500;margin-bottom:8px">📤 Share with Another Coach</button></div>';
  const playerContent='<div style="padding:4px 0"><div style="background:linear-gradient(135deg,#1A3A5C,#0C2340);border-radius:12px;padding:20px 18px;margin-bottom:16px;text-align:center"><div style="font-size:32px;margin-bottom:8px">🎯</div><div style="font-size:20px;font-weight:500;color:#FFD700;margin-bottom:6px">Sharpshooter</div><div style="font-size:12px;color:#ccc;font-style:italic">"What gets measured, improves"</div></div><div class="card" style="border-left:3px solid #FFD700"><div style="font-size:14px;font-weight:500;color:#1A3A5C;margin-bottom:8px">The best players track their work.</div><div style="font-size:13px;color:#444;line-height:1.7">Log your makes, climb the ranks, compete for the crown, and get feedback from your coach.</div></div><div class="card"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:10px">📱 How It Works</div>'+playerSteps.map(function(s,i){return'<div style="display:flex;gap:10px;margin-bottom:12px;align-items:flex-start"><div style="font-size:16px;min-width:24px;font-weight:500;color:#1A3A5C">'+(i+1)+'.</div><div><div style="font-size:13px;font-weight:500;color:#1A3A5C">'+s[0]+'</div><div style="font-size:12px;color:#555;line-height:1.5;margin-top:2px">'+s[1]+'</div></div></div>';}).join("")+'</div><div class="card" style="background:linear-gradient(135deg,#2a1a00,#1a0f00);border:1.5px solid #FFD700"><div style="font-size:13px;font-weight:500;color:#FFD700;margin-bottom:8px">👑 What is the Shooting King?</div><div style="font-size:12px;color:#aaa;line-height:1.7">The player with the most <strong style="color:#FFD700">weighted makes</strong> in a week. Pull-up threes count <strong style="color:#FFD700">4x</strong> more than form shooting.</div></div><div class="card"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:4px">🏅 The Rank Ladder</div><div style="font-size:11px;color:#888;margin-bottom:10px">Earn King Points each week to climb. Resets every Monday — earn it again.</div>'+rankLadder+'</div><div class="card" style="background:linear-gradient(135deg,#FFF3E0,#fff);border:1px solid #FFB74D"><div style="font-size:13px;font-weight:500;color:#E65100;margin-bottom:8px">🔥 Streaks</div><div style="font-size:12px;color:#444;line-height:1.7">Your streak is how many days in a row you have logged a workout. Miss a day and it resets — so keep showing up. The Streaks board shows who is the most consistent on the team.</div></div><div class="card"><div style="font-size:13px;font-weight:500;color:#1A3A5C;margin-bottom:8px">💬 Coach Feedback</div><div style="font-size:12px;color:#444;line-height:1.7">Check your <strong>Summary screen</strong> to see notes from your coach. Private — only you see it.</div></div></div>';
  return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:14px"><button data-action="go-home">← Back</button><span style="font-weight:500;font-size:15px">ℹ️ About Sharpshooter</span></div><div style="display:flex;gap:6px;margin-bottom:14px"><button data-action="about-tab" data-t="coaches" class="'+(aboutTab==="coaches"?"btn-primary":"")+'" style="flex:1;padding:10px;font-size:13px;font-weight:500">🏀 For Coaches</button><button data-action="about-tab" data-t="players" class="'+(aboutTab==="players"?"btn-primary":"")+'" style="flex:1;padding:10px;font-size:13px;font-weight:500">👤 For Players</button></div>'+(aboutTab==="coaches"?coachContent:playerContent);
}
...
async function handleCreateTeam() {
  const name=(document.getElementById("new-team-name")?.value||"").trim(),code=(document.getElementById("new-team-code")?.value||"").trim().toUpperCase(),pin=(document.getElementById("new-team-pin")?.value||"").trim(),msg=document.getElementById("create-err");
...
function attachEvents() {
  document.body.addEventListener("click",function(e){const bv=e.target.closest("[data-action='close-video']");if(bv){const overlay=bv.closest("div[style*='position:fixed']");if(overlay)overlay.remove();}});
  document.getElementById("app").addEventListener("click", async e => {
    ...
    if(a==="sel-player"){const name=b.dataset.name;curPlayer=name;localEdits={};const existingPin=getPlayerPin(name);if(isPlayerUnlocked(name,teamCode)){screen="player";render(buildPlayer());}else if(existingPin===null){pinMode="player-setup";pinEntry="";pinErr="";pinSetupFirst="";screen="pin";render(buildPin());}else{pinMode="player-enter";pinEntry="";pinErr="";screen="pin";render(buildPin());}}
    if(a==="go-home"){screen="home";coachOpen=false;coachViewPlayer=null;render(buildHome());}
    if(a==="play-video"){document.body.insertAdjacentHTML("beforeend", buildVideoPlayer(b.dataset.vid));}
    if(a==="close-video"){const el=document.querySelector("[data-action='close-video']");if(el)el.closest("div[style*='position:fixed']").remove();}
    ...
    if(a==="save-player-cats"){if(b.disabled)return;const player=b.dataset.player;b.disabled=true;b.textContent="Saving...";await initPlayerCategories(player);const pRows=allPlayerCategories.filter(function(c){return c.player===player&&c.team_code===teamCode;});for(var i=0;i<pRows.length;i++){const ne=document.getElementById("pcat-name-"+i),we=document.getElementById("pcat-weight-"+i);if(ne&&we){const n2=ne.value.trim(),w2=parseFloat(we.value)||1.0;if(n2)await savePlayerCategory(player,pRows[i].id,n2,w2,i);}}showToast("Categories saved for "+player+"!");render(buildCoach());}
    if(a==="add-player-cat"){const player=b.dataset.player,ne=document.getElementById("new-pcat-name"),we=document.getElementById("new-pcat-weight"),n2=(ne?ne.value:"").trim(),w2=parseFloat(we?we.value:1)||1.0;if(!n2){showToast("Enter a category name.");return;}b.disabled=true;b.textContent="Adding...";await initPlayerCategories(player);const so=allPlayerCategories.filter(function(c){return c.player===player&&c.team_code===teamCode;}).length;await savePlayerCategory(player,null,n2,w2,so);showToast("Category added for "+player+"!");render(buildCoach());}
    if(a==="delete-player-cat"){const player=b.dataset.player,id=b.dataset.id,idx=parseInt(b.dataset.idx),pRows=allPlayerCategories.filter(function(c){return c.player===player&&c.team_code===teamCode;}),catName=pRows[idx]?pRows[idx].name:"this category";if(!confirm("Remove "+catName+" from "+player+"?"))return;if(id)await deletePlayerCategory(id);showToast("Category removed.");render(buildCoach());}
    if(a==="reset-player-cats"){const player=b.dataset.player;if(!confirm("Reset "+player+" to team default categories? Their custom categories will be deleted."))return;await db.from("player_categories").delete().eq("player",player).eq("team_code",teamCode);allPlayerCategories=allPlayerCategories.filter(function(c){return!(c.player===player&&c.team_code===teamCode);});showToast("Reset to team defaults.");render(buildCoach());}
    if(a==="save-categories"){if(b.disabled)return;b.disabled=true;b.textContent="Saving...";if(allTeamCategories.length===0)await initTeamCategories();for(let i=0;i<allTeamCategories.length;i++){const nameEl=document.getElementById("cat-name-"+i),weightEl=document.getElementById("cat-weight-"+i);if(nameEl&&weightEl){const name=nameEl.value.trim(),weight=parseFloat(weightEl.value)||1.0;if(name)await saveTeamCategory(allTeamCategories[i].id,name,weight);}}showToast("✓ Categories saved!");render(buildCoach());}
    if(a==="add-category"){if(b.disabled)return;const nameEl=document.getElementById("new-cat-name"),weightEl=document.getElementById("new-cat-weight"),msg=document.getElementById("cat-msg"),name=(nameEl?.value||"").trim(),weight=parseFloat(weightEl?.value)||1.0;if(!name){if(msg)msg.innerHTML='<span class="err">Enter a category name.</span>';return;}if(allTeamCategories.find(function(c){return c.name.toLowerCase()===name.toLowerCase();})){if(msg)msg.innerHTML='<span class="err">Category already exists.</span>';return;}b.disabled=true;b.textContent="Adding...";if(allTeamCategories.length===0)await initTeamCategories();await saveTeamCategory(null,name,weight);showToast("✓ Category added!");render(buildCoach());}
    if(a==="delete-category"){const idx=parseInt(b.dataset.idx),id=b.dataset.id,catName=allTeamCategories[idx]?allTeamCategories[idx].name:"this category";if(!confirm("Delete "+catName+"? Player data for this category is kept but won't show on the entry screen."))return;if(id)await deleteTeamCategory(id);showToast("Category removed.");render(buildCoach());}
    if(a==="send-message"){if(b.disabled)return;const text=(document.getElementById("msg-text")?.value||"").trim(),target=(document.getElementById("msg-target")?.value||"").trim();if(!text){showToast("Type a message first.");return;}b.disabled=true;b.textContent="Sending...";const err=await sendMessage(text,target||null);if(err){showToast("Error sending message.");b.disabled=false;b.textContent="Send";return;}showToast("Sent"+(target?" to "+target:"to everyone")+"!");render(buildCoach());}
    if(a==="delete-message"){if(!confirm("Delete this message?"))return;await deleteMessage(b.dataset.id);render(buildCoach());}
    if(a==="dismiss-message"){await markMessageRead(b.dataset.id, curPlayer);render(buildPlayer());}
    ...
    if(a==="save-coach-comment"){const type=b.dataset.type,week=b.dataset.week||null,dayVal=b.dataset.day,day=dayVal!==""?parseInt(dayVal):null;let text="";if(type==="player")text=(document.getElementById("coach-player-note")?.value||"").trim();if(type==="week")text=(document.getElementById("coach-week-note")?.value||"").trim();if(type==="day")text=(document.getElementById("coach-day-note")?.value||"").trim();b.disabled=true;b.textContent="Saving...";await saveCoachComment(coachViewPlayer,type,week,day,text);showToast("✓ Comment saved!");b.disabled=false;b.textContent=`Save ${type.charAt(0).toUpperCase()+type.slice(1)} Note`;}
    if(a==="reset-player-pin"){const name=b.dataset.name;if(!confirm(`Reset PIN for ${name}? They'll set a new one next time they log in.`))return;await db.from("player_pins").delete().eq("player",name).eq("team_code",teamCode);allPlayerPins=allPlayerPins.filter(p=>!(p.player===name&&p.team_code===teamCode));try{const u=getUnlockedPlayers();delete u[teamCode+"|"+name];sessionStorage.setItem("bball_unlocked",JSON.stringify(u));}catch(e){}showToast(`PIN reset for ${name}`);render(buildCoach());}
    if(a==="add-player"){if(b.disabled)return;const inp=document.getElementById("np"),name=(inp?.value||"").trim(),msg=document.getElementById("rmsg");if(!name){if(msg)msg.innerHTML=`<span class="err">Enter a name.</span>`;return;}if(roster.includes(name)){if(msg)msg.innerHTML=`<span class="err">Already on roster.</span>`;return;}b.disabled=true;b.textContent="Adding...";const err=await addPlayerToDB(name);if(err){if(msg)msg.innerHTML=`<span class="err">${err}</span>`;b.disabled=false;b.textContent="Add";return;}render(buildCoach());}
    if(a==="rm-player"){const pname=b.dataset.name;if(!confirm(`⚠️ Remove ${pname}?\n\nThis will permanently delete ALL their shots, notes, check-ins, and PIN. This cannot be undone.`))return;if(!confirm(`Are you sure? All data for ${pname} will be gone forever.`))return;await removePlayerFromDB(pname);render(buildCoach());}
    if(a==="save-pin"){const v=(document.getElementById("npin")?.value||"").trim(),msg=document.getElementById("pmsg");if(!/^\d{4}$/.test(v)){if(msg)msg.innerHTML=`<span class="err">Must be 4 digits.</span>`;return;}await savePinToDB(v);if(msg)msg.innerHTML=`<span class="ok">PIN updated.</span>`;}
    ...
    if(a==="save-wcheckin"){if(b.disabled)return;b.disabled=true;const wk=weekKey(),existing=getWeeklyCheckin(curPlayer,wk)||{};const align=parseInt(document.getElementById("alignment-slider")?.value)||existing.alignment||5;await saveWeeklyCheckin(curPlayer,wk,align,checkinTemp.confidence??existing.confidence??null,checkinTemp.selftalk??existing.selftalk??null);checkinTemp={};showToast("✓ Weekly check-in saved!");screen="summary";render(buildSummary());}
...
async function refreshData() {
  if(!teamCode) return;
  await loadShots();await loadNotes();await loadSpotNames();await loadSpotCounts();await loadDailyCheckins();await loadWeeklyCheckins();await loadPlayerPins();await loadTeamCategories();await loadPlayerCategories();await loadCoachComments();await loadMessages();await loadMessageReads();
