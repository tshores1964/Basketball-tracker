...
function buildLeaderboard() {
  try {
    ...
    function sbRows(rows,isAtt,isDiff){isAtt=!!isAtt;isDiff=!!isDiff;if(!rows.length)return'<div class="sb-no-data">No data yet \u2014 get to work! \uD83C\uDFC0</div>';const max=rows[0].val||1;return rows.map(function(r,i){const barW=Math.round((r.val/max)*100),valStr=isDiff?(r.val>0?"+":"")+r.val+"%":isAtt?String(r.val):r.val+"%",medalClass=i<3?"medal-"+(i+1):"";return'<div class="sb-row '+medalClass+'"><div class="sb-rank '+rankMedal(i)+'">'+rankSymbol(i)+'</div><div class="sb-avatar">'+initials(r.name)+'</div><div class="sb-name">'+r.name+'</div><div class="sb-bar-wrap"><div class="sb-bar" style="width:'+barW+'%"></div></div><div><div class="sb-stat">'+valStr+'</div><div class="sb-sub">'+r.sub+'</div></div></div>';}).join("");}
    ...
    const sectionIds=["overall","attempts","bestday","improved","cats"],sectionLabels=["Overall %","Most Shots","Best Day","Improved","By Category"];
...
function buildAbout() {
  const coachFeatures=[["Shot Tracking","Players log makes and attempts by category, spot, and day — every workout, every week."],["Shooting King","A weighted leaderboard crowns the week's best shooter. 3-point makes count more. Pull-ups count more."],["Mental Performance","Daily and weekly check-ins built on CMPT principles — effort, recovery, confidence, self-talk."],["Coach Comments","Leave feedback per player, per week, or per day. Only that player sees it."],["Player Summaries","Week, month, and year stats. Best spot. Progress chart by category. Trend vs last week."],["League Competition","Opt your team into cross-team competition. Weekly totals compete against other Sharpshooter teams."],["Player PINs","Each player protects their own data with a personal PIN. Coach has master override."]];
